...
R=Path(__file__).resolve().parents[1];G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII';O=R/'coverage/display-settings';E=O/'runtime';E.mkdir(exist_ok=True)
...
results=[]
for label,mode,width,scale,filtering,vsync in [('crisp-windowed','windowed',640,1,'nearest','on'),('higher-borderless','borderless',960,2,'bilinear','off')]:
 ...
 with (out/'stdout.log').open('w') as stdout,(out/'stderr.log').open('w') as stderr:
  game=subprocess.Popen([str(E/'game.exe'),'--game',str(G/'game.toml'),'--debug-port','4387','--memcard-dir',str(out/'saves')],cwd=out,stdout=stdout,stderr=stderr,creationflags=subprocess.CREATE_NO_WINDOW)
  try:
   probe=subprocess.run([sys.executable,str(R/'scripts/probe_display_settings.py'),'--out',str(out),'--load-save'],capture_output=True,text=True,timeout=240)
...
(O/'runtime-validation.json').write_text(json.dumps({'runs':results,'original_cards_unchanged':True,'scope':'Actual OpenGL windowed and borderless runs through copied-save load, movement, palette cases and final framebuffer capture.'},indent=2)+'\n')
