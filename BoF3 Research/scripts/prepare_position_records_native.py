...
R=Path(__file__).resolve().parents[1];O=R/'coverage/position-records-native';O.mkdir(parents=True,exist_ok=True)
runpy.run_path(str(R/'scripts/prepare_action_checks_native.py'),run_name='__main__')
inputs=json.loads((R/'startup-action-checks-inputs.json').read_text());manifest=[]
...
targets=[(0x801c472c,0x801c476c,'bof3_check_position_attribute'),(0x801c476c,0x801c48fc,'bof3_check_fractional_cell_attribute'),(0x801bdab8,0x801bdb7c,'bof3_process_primary_records_reverse'),(0x801a7bf0,0x801a7c2c,'bof3_dispatch_secondary_scene_mode')]
for lo,hi,name in targets:
 ...
 expected_prefix={0x801c472c:(0x27bdffe8,0x93a20028,0x10c00006),0x801c476c:(0x27bdffc8,0xafb40028,0x0080a021),0x801bdab8:(0x3c028014,0x90426254,0x27bdffd8),0x801a7bf0:(0x3c028014,0x804248ea,0x27bdffe8)}
 assert struct.unpack_from('<III',code)==expected_prefix[lo]
 assert struct.unpack_from('<II',code,len(code)-8)==(0x03e00008,0x27bd0018 if lo==0x801c472c else 0)
 ...
 assert indirect==(['0x801A7C14'] if lo==0x801a7bf0 else [])
