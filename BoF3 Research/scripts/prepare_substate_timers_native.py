...
R=Path(__file__).resolve().parents[1];O=R/'coverage/substate-timers-native';O.mkdir(parents=True,exist_ok=True)
runpy.run_path(str(R/'scripts/prepare_state_boundaries_native.py'),run_name='__main__')
inputs=json.loads((R/'startup-state-boundaries-inputs.json').read_text());manifest=[]
...
targets=[(0x801b036c,0x801b0568,'bof3_update_primary_substate0_actions'),(0x801a253c,0x801a271c,'bof3_advance_scene_record_motion'),(0x80198cac,0x80198f1c,'bof3_tick_play_clock_and_countdowns')]
for lo,hi,name in targets:
 ...
 expected_prefix={0x801b036c:(0x3c028014,0x94426258,0x27bdffe8),0x801a253c:(0x3c061f80,0x8cc60044,0x27bdffe8),0x80198cac:(0x3c038014,0x90633bb0,0x24020009)}
 ...
 if lo==0x801b036c:assert '0x801CD0E8' in pointers
...
(R/'startup-substate-timers-inputs.json').write_text(json.dumps(inputs,indent=2)+'\n');(O/'recipes.json').write_text(json.dumps({'routines':manifest,'total_new_instruction_bytes':sum(x['bytes'] for x in manifest)},indent=2)+'\n')
print('Prepared substate/motion/timer helpers:',sum(x['bytes'] for x in manifest),'instruction bytes')
