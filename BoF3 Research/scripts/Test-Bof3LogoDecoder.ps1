...
$runRoot=if($EvidenceRoot){Join-Path $EvidenceRoot $Variant}else{Join-Path $workspace "BoF3 Research\coverage\logo-decoder-native\$Variant"}
New-Item -ItemType Directory -Force -Path "$runRoot\saves"|Out-Null
$exe=if($Variant -eq 'baseline'){"$gameRoot\build-release\Breath_of_Fire_III___PSXRecomp.before-logo-decoder.exe"}else{"$gameRoot\build-release\Breath_of_Fire_III___PSXRecomp.exe"}
...
$previousRing=$env:PSX_DISPLAY_RING
...
try {
 ...
 & "$env:USERPROFILE\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" "$PSScriptRoot\probe_logo_decoder.py" @probeArgs *> "$runRoot\probe.log"
 ...
 if($null -eq $previousRing){Remove-Item Env:PSX_DISPLAY_RING -ErrorAction SilentlyContinue}else{$env:PSX_DISPLAY_RING=$previousRing}
