...
p=argparse.ArgumentParser();p.add_argument('mode',choices=['baseline','native','original','missing']);p.add_argument('--wide',action='store_true');p.add_argument('--scale',type=int,default=1);p.add_argument('--tag',default='');p.add_argument('--world',action='store_true');p.add_argument('--hold',type=int,default=0);a=p.parse_args()
R=Path(__file__).resolve().parents[1];G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII';O=R/'coverage/title-footer';D=O/(a.mode+('-wide' if a.wide else '-plain')+f'-{a.scale}x'+('-'+a.tag if a.tag else ''));E=D/'runtime';E.mkdir(parents=True,exist_ok=True)
...
exe=R/'coverage/widescreen-terrain-release/baseline.exe' if a.mode=='baseline' else G/'build-release/Breath_of_Fire_III___PSXRecomp.exe';shutil.copy2(exe,E/'game.exe')
for n in ['assets','bios','mods']:shutil.copytree(G/'build-release'/n,E/n,dirs_exist_ok=True)
if a.mode=='missing':(E/'assets/title/press-start.png').rename(E/'assets/title/press-start.png.unavailable')
...
for p in originals:shutil.copy2((R/'coverage/widescreen-culling/runtime/saves'/Path(p).name) if a.world else p,D/'saves'/Path(p).name)
def call(c,**kw):
...
with (D/'stdout.log').open('w') as out,(D/'stderr.log').open('w') as err:
 ...
 try:
  ...
  if a.world:
   for delay in [350,100,1000]:call('press',buttons=0xbfff,frames=5);wait(delay)
   snap('loaded');assert int.from_bytes(ram(0x80143b92,2),'little')==0
  if a.hold:
   print('READY FOR DIAGNOSTICS',game.pid,flush=True)
   end=time.monotonic()+a.hold
   while time.monotonic()<end and game.poll() is None and not (D/'finish').exists():time.sleep(.2)
