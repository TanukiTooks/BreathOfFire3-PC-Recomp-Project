...
while True:
 try:frame();break
  ...
  time.sleep(.1)
call('wtrace_range',lo='0x80146872',hi='0x80146876')
...
def capture(label):
 ...
 (a.out/(label+'-context.json')).write_text(json.dumps({'party_count':ram(0x80146254,1)[0],'scenario_state':list(ram(0x80146870,8)),'scenario_tables':ram(0x801fe29c,108).hex(),'scene_mode':ram(0x80146870,1)[0],'map':ram(0x80143f00,4).hex(),'clock':list(ram(0x80144fc0,4))},indent=2))
 return s
...
(a.out/'scenario-write-trace.json').write_text(json.dumps(call('wtrace_dump',count=2048,addr_lo='0x80146872',addr_hi='0x80146876'),indent=2))
