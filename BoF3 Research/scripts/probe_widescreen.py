...
def present(label):
 seq=call('present_shot_seq')['seq'];call('present_shot',path=(a.out/(label+'-present.png')).resolve().as_posix())
...
while True:
 try:frame();break
  ...
  time.sleep(.1)
for target in [800,1600,3000]:
 while frame()<target:time.sleep(.05)
 present('boot-'+str(target))
