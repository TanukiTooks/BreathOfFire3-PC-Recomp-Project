...
R=Path(__file__).resolve().parents[1];G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII';O=R/'coverage/party-motion-native'
p=argparse.ArgumentParser();p.add_argument('variant',choices=['baseline','native']);p.add_argument('--repeat-baseline',action='store_true');a=p.parse_args();assert not a.repeat_baseline or a.variant=='baseline';D=O/('baseline-repeat' if a.repeat_baseline else a.variant);E=D/'runtime';E.mkdir(parents=True,exist_ok=True)
...
with (D/'stdout.log').open('w') as out,(D/'stderr.log').open('w') as err:
 ...
 try:
  probe=subprocess.run([sys.executable,str(R/'scripts/probe_party_motion.py'),'--out',str(D),'--load-save'],capture_output=True,text=True,timeout=300)
