...
R=Path(__file__).resolve().parents[1];O=R/'coverage/panel-border-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':1168,'checkpoints':[],'limitations':['Eight headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
for entry in ['0x801E5320','0x801DE088','0x801DF21C','0x801E14A8']:assert counts.get(entry,0)>=8
new=(G/'generated-panel-border-overlays/overlays_static.c').read_text();old=(G/'generated-card-drawing-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==32
...
old_inputs=json.loads((R/'startup-card-drawing-inputs.json').read_text());new_inputs=json.loads((R/'startup-panel-border-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+2
...
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-panel-border.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
(O/'validation.json').write_text(json.dumps(result,indent=2)+'\n')
print('PASS: two guarded native border helpers, preserved prior entries and inputs, eight matching state checkpoints, screenshot comparison, copied-save load, Ghidra names and unchanged original cards.')
