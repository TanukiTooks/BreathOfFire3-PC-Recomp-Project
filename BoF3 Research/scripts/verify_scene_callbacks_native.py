...
R=Path(__file__).resolve().parents[1];O=R/'coverage/scene-callbacks-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':1792,'checkpoints':[],'limitations':['Twelve headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
new=(G/'generated-scene-callbacks-overlays/overlays_static.c').read_text();old=(G/'generated-gameplay-loops-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==86
...
old_inputs=json.loads((R/'startup-gameplay-loops-inputs.json').read_text());new_inputs=json.loads((R/'startup-scene-callbacks-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+3
...
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-scene-callbacks.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
assert 'SCENE_MOTION_BOUNDARY_MERGED=1' in (O/'ghidra-boundary.log').read_text()
result['hotspots']={}
for f in manifest['routines']:
...
for f in manifest['routines']:
 ...
 for target in [f['entry']]+f['observed_hotspots']+f['case_dispatch_entries']:
...
assert len(case_checks)==10 and counts.get('0x801975D0',0)>=8
result['state_cases']=case_checks
source=(R/'coverage/ghidra-input/GAME-section-00-80195800.bin').read_bytes()[0x30:0x58]
for variant in ['baseline','native']:assert (O/variant/'live-state-jump-table.bin').read_bytes()==source
result['state_jump_table_matches']=True
...
print('PASS: three guarded native scene callbacks and verified case entries, preserved prior entries and inputs, twelve matching state checkpoints, screenshot comparison, copied-save load, Ghidra names and unchanged original cards.')
