...
$runRoot=if($EvidenceRoot){Join-Path $EvidenceRoot $Variant}else{Join-Path $workspace "BoF3 Research\coverage\state-boundaries-native\$Variant"}
New-Item -ItemType Directory -Force -Path "$runRoot\saves"|Out-Null
$exe=if($Variant -eq 'baseline'){"$gameRoot\build-release\Breath_of_Fire_III___PSXRecomp.before-state-boundaries.exe"}else{"$gameRoot\build-release\Breath_of_Fire_III___PSXRecomp.exe"}
...
try {
 ...
 & "$env:USERPROFILE\.cache\codex-runtimes\codex-primary-runtime\dependencies\python\python.exe" "$PSScriptRoot\probe_state_boundaries.py" @probeArgs *> "$runRoot\probe.log"
