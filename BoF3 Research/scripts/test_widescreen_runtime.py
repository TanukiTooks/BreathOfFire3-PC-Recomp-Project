...
results=[]
for label,mode,width,scale,filtering,vsync in [('native-4-3','windowed',960,1,'nearest','on'),('legacy-16-9','windowed',1280,1,'nearest','on')]:
...
(O/'runtime-validation.json').write_text(json.dumps({'runs':results,'original_cards_unchanged':True,'scope':'Actual OpenGL native 4:3 and Legacy-configured 16:9 runs through copied-save load, movement, palette cases and final framebuffer capture.'},indent=2)+'\n')
