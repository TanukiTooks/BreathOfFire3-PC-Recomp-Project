...
R=Path(__file__).resolve().parents[1];O=R/'coverage/primary-motion-native';O.mkdir(parents=True,exist_ok=True)
runpy.run_path(str(R/'scripts/prepare_frame_helpers_native.py'),run_name='__main__')
inputs=json.loads((R/'startup-frame-helpers-inputs.json').read_text());manifest=[]
...
targets=[(0x801b00f8,0x801b016c,'bof3_dispatch_primary_record_state'),(0x801a8d38,0x801a8f94,'bof3_dispatch_scene_motion_command'),(0x80196c8c,0x80196cf0,'bof3_update_active_sequence_records')]
for lo,hi,name in targets:
 ...
 assert indirect==(['0x801B0154'] if lo==0x801b00f8 else [])
...
(R/'startup-primary-motion-inputs.json').write_text(json.dumps(inputs,indent=2)+'\n');(O/'recipes.json').write_text(json.dumps({'routines':manifest,'total_new_instruction_bytes':sum(x['bytes'] for x in manifest)},indent=2)+'\n')
print('Prepared primary/motion helpers:',sum(x['bytes'] for x in manifest),'instruction bytes')
