...
R=Path(__file__).resolve().parents[1];O=R/'coverage/action-checks-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':848,'checkpoints':[],'limitations':['Twelve headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
new=(G/'generated-action-checks-overlays/overlays_static.c').read_text();old=(G/'generated-substate-timers-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==35
...
old_inputs=json.loads((R/'startup-substate-timers-inputs.json').read_text());new_inputs=json.loads((R/'startup-action-checks-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+4
...
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-action-checks.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
prior_validation=json.loads((R/'coverage/substate-timers-native/validation.json').read_text())
...
boundary_log=(O/'ghidra-GAME.log').read_text();assert 'ACTION_CHECK_BOUNDARIES_MERGED=2' in boundary_log
...
result['shared_helper_attribution']={}
for variant in ['baseline','native']:
 stats=json.loads((O/variant/'gameplay-settled.json').read_text())['dirty']['per_pc'];result['shared_helper_attribution'][variant]=next((x['insns'] for x in stats if int(x['pc'],16)&0x1fffffff==0x001c364c),0)
assert result['shared_helper_attribution']['native']==0
...
print('PASS: four guarded native action-check helpers, prior entries and inputs retained, twelve state checkpoints, screenshot comparison, copied-save load, four active palette cases, Ghidra names and unchanged original cards.')
