...
R=Path(__file__).resolve().parents[1];O=R/'coverage/motion-commands-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':3544,'checkpoints':[],'limitations':['Twelve headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
new=(G/'generated-motion-commands-overlays/overlays_static.c').read_text();old=(G/'generated-position-records-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==166
...
old_inputs=json.loads((R/'startup-position-records-inputs.json').read_text());new_inputs=json.loads((R/'startup-motion-commands-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+2
...
for f in manifest['routines']:
 ...
 assert 'RESEARCH_SYMBOLS_APPLIED=2' in (O/f"ghidra-{f['section']}.log").read_text()
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-motion-commands.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
prior_validation=json.loads((R/'coverage/position-records-native/validation.json').read_text())
# Rare gated returns vary across polling-timed runs; retain presence, not an exact historical count.
for entry,count in prior_validation['native_entry_counts_at_least'].items():assert counts.get(entry,0)>=(8 if count>=8 else 1),entry
result['prior_entry_validation']='Previously frequent entries remain >=8; rare gated entries remain observed at least once. Exact counts are not synchronized.'
result['hotspots']={}
for f in manifest['routines']:
...
for f in manifest['routines']:
 ...
 for target in [f['entry']]+f['observed_hotspots']+f['resume_dispatch_entries']+f['case_dispatch_entries']:
...
assert 'Save succeeded' in (O/'ghidra-GAME.log').read_text()
result['dispatch_contexts']=[]
for stage in ['gameplay-idle','gameplay-right','gameplay-left','gameplay-settled']:
 a=json.loads((O/'baseline'/f'{stage}-dispatch-context.json').read_text());b=json.loads((O/'native'/f'{stage}-dispatch-context.json').read_text());assert a==b,(stage,a,b)
 for address,target in [('primary_table_entry','primary_target'),('mode_table_entry','mode_pointer'),('flag_helper_table_entry','flag_helper_target')]:assert struct.unpack_from('<I',game,int(a[address],16)-0x80195800)[0]==int(a[target],16)
 result['dispatch_contexts'].append(dict(stage=stage,**a))
result['limitations']+=['Observed state/substate/mode pointer values establish this route only; other callback selections and both flag-helper predicate outcomes are not individually verified.','The mode dispatcher dereferences a live pointer outside GAME; the pointed-to callback retains its prior execution path and needs its own code-image identification.']
result['timer_observations']={}
def clock_ticks(b):
 assert len(b)==4 and 0<=b[0]<=99 and 0<=b[1]<60 and 0<=b[2]<60 and 0<=b[3]<30,b
 return ((b[0]*60+b[1])*60+b[2])*30+b[3]
for variant in ['baseline','native']:
 samples=[dict(stage=stage,**json.loads((O/variant/f'{stage}-timers.json').read_text())) for stage in ['gameplay-idle','gameplay-right','gameplay-left','gameplay-settled']]
 for sample in samples:
  assert sample['before_frame']<=sample['after_frame'] and sample['phase']==0
  sample['play_clock_ticks']=clock_ticks(sample['play_clock_bytes']);assert sample['countdown_bytes']==[0]*8
 intervals=[]
 for a,b in zip(samples,samples[1:]):
  ticks=b['play_clock_ticks']-a['play_clock_ticks'];low=b['before_frame']-a['after_frame'];high=b['after_frame']-a['before_frame']
  assert ticks>0 and low-1<=ticks*2<=high+1,(variant,a,b,ticks,low,high)
  intervals.append({'from':a['stage'],'to':b['stage'],'clock_tick_delta':ticks,'runtime_frame_delta_bounds':[low,high],'one_tick_per_two_runtime_frames_within_sampling_bounds':True})
 assert samples[-1]['play_clock_bytes'][1]>samples[0]['play_clock_bytes'][1]
 result['timer_observations'][variant]={'samples':samples,'intervals':intervals,'minute_boundary_crossed':True,'countdowns_inactive':True}
result['limitations']+=['Play clock minute rollover and sampled rate are checked; active countdowns, phase-9 suppression, hour rollover and maximum-time saturation remain untested.','Observed timer-to-debug-frame relationship is route-specific evidence, not a displayed-FPS benchmark.']
result['position_contexts']=[]
for stage in ['gameplay-idle','gameplay-right','gameplay-left','gameplay-settled']:
 a=json.loads((O/'baseline'/f'{stage}-position-context.json').read_text());b=json.loads((O/'native'/f'{stage}-position-context.json').read_text());assert a==b,(stage,a,b)
 assert a['primary_footprint_selector']==0 and a['primary_count']==1
 assert struct.unpack_from('<I',game,int(a['secondary_table_entry'],16)-0x80195800)[0]==int(a['secondary_target'],16)
 result['position_contexts'].append(dict(stage=stage,**a))
result['limitations']+=['Fractional-footprint selector 0 and one active primary record are observed; the alternate extended-neighbor helper, multiple-record/skip paths and all attribute-match/mask combinations are not validated.','Secondary mode 1 callback identity is observed via the original table; other modes remain untested.']
result['command_case_dispatch']=case_checks
result['command_jump_tables']=[]
for table in manifest['command_jump_tables']:
 address=int(table['address'],16);data=game[address-0x80195800:address-0x80195800+table['bytes']]
 assert hashlib.sha256(data).hexdigest()==table['sha256']
 assert [f'0x{x:08X}' for x in struct.unpack('<'+'I'*(len(data)//4),data)]==table['targets']
 for variant in ['baseline','native']:assert (O/variant/f'live-command-table-{address:08X}.bin').read_bytes()==data
 result['command_jump_tables'].append(dict(table,original_table_matches_both_runs=True,observed_commands=[hex(table['first_command']+i) for i,pc in enumerate(table['targets']) if counts.get(pc,0)>0]))
for pc,word in manifest['verified_switch_opcodes'].items():assert struct.unpack_from('<I',game,int(pc,16)-0x80195800)[0]==int(word,16)
assert len(case_checks)==31
result['limitations']+=['All 31 switch-target variants have verified guards, but only logged commands are observed executing. Dynamic callbacks, script loops/conditions, extended motion parameters and division-trap paths need targeted scenarios.','Preserving source instructions and table loads does not establish that every command behaves correctly at runtime.']
fix=json.loads((O/'break-translation-fix.json').read_text(encoding='utf-8'));fixed=(G/fix['generated_file']).read_text(encoding='utf-8');assert len(fix['sites'])==8
for site in fix['sites']:
 assert struct.unpack_from('<I',game,int(site['pc'],16)-0x80195800)[0]==int(site['opcode'],16)
 assert fixed.count(site['generated_statement'])==1
assert 'trap, no-op in recompiler' not in fixed
result['break_translation_fix']=fix
result['limitations']+=['Eight BREAK translations now call the same terminating trap helper as the dirty-RAM baseline; invalid division scenarios are not exercised here and full hardware exception emulation is not implemented.']
(O/'validation.json').write_text(json.dumps(result,indent=2)+'\n')
print('PASS: two guarded native motion-command interpreters and 31 guarded case targets, prior entries and inputs retained, twelve state checkpoints, screenshot comparison, copied-save load, four active palette cases, Ghidra names and unchanged original cards.')
