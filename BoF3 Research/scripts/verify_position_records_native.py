...
R=Path(__file__).resolve().parents[1];O=R/'coverage/position-records-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':720,'checkpoints':[],'limitations':['Twelve headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
new=(G/'generated-position-records-overlays/overlays_static.c').read_text();old=(G/'generated-action-checks-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==39
...
old_inputs=json.loads((R/'startup-action-checks-inputs.json').read_text());new_inputs=json.loads((R/'startup-position-records-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+4
...
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-position-records.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
prior_validation=json.loads((R/'coverage/action-checks-native/validation.json').read_text())
...
boundary_log=(O/'ghidra-GAME.log').read_text();assert 'POSITION_RECORD_BOUNDARIES_MERGED=2' in boundary_log
...
result['footprint_helper_attribution']={}
for variant in ['baseline','native']:
 stats=json.loads((O/variant/'gameplay-settled.json').read_text())['dirty']['per_pc'];result['footprint_helper_attribution'][variant]=next((x['insns'] for x in stats if int(x['pc'],16)&0x1fffffff==0x001c476c),0)
assert result['footprint_helper_attribution']['native']==0
result['limitations']+=['Shared helper native execution and guards are verified; an absent standalone baseline hotspot is not evidence that the helper previously did no interpreted work.','Specific actions 0x89/0x8A, successful position-trigger paths, alternate direction searches and idle random/party paths require broader gameplay evidence.']
...
print('PASS: four guarded native position/record helpers, prior entries and inputs retained, twelve state checkpoints, screenshot comparison, copied-save load, four active palette cases, Ghidra names and unchanged original cards.')
