...
R=Path(__file__).resolve().parents[1];O=R/'coverage/state-boundaries-native';G=R.parent/'BoF3 PSXRecomp/BreathOfFireIII'
manifest=json.loads((O/'recipes.json').read_text())
result={'status':'passed','new_routines':[],'new_instruction_bytes':204,'checkpoints':[],'limitations':['Twelve headless checkpoints; not all menus, input configurations, graphics backends or full-game paths.','Counts are bounded-log lower bounds, not exact totals or speed measurements.','Native dispatch guards and entry execution are verified; dependencies keep their existing routing.']}
...
new=(G/'generated-state-boundaries-overlays/overlays_static.c').read_text();old=(G/'generated-primary-motion-overlays/overlays_static.c').read_text();pattern=r'^void (\w+)\(CPUState \*cpu\);';old_ids=set(re.findall(pattern,old,re.M));new_ids=set(re.findall(pattern,new,re.M));assert old_ids and old_ids<=new_ids
added=new_ids-old_ids;assert len(added)==9
...
old_inputs=json.loads((R/'startup-primary-motion-inputs.json').read_text());new_inputs=json.loads((R/'startup-state-boundaries-inputs.json').read_text());assert new_inputs[:len(old_inputs)]==old_inputs and len(new_inputs)==len(old_inputs)+3
...
for label,filename in [('baseline','Breath_of_Fire_III___PSXRecomp.before-state-boundaries.exe'),('native','Breath_of_Fire_III___PSXRecomp.exe')]:result[label+'_exe_sha256']=hashlib.sha256((G/'build-release'/filename).read_bytes()).hexdigest()
prior_validation=json.loads((R/'coverage/primary-motion-native/validation.json').read_text())
...
boundary_log=(O/'ghidra-GAME.log').read_text();assert 'STATE_DISPATCH_BOUNDARY_MERGED=1' in boundary_log and 'RECORD_FLAG_LEAF_CREATED=1' in boundary_log
...
print('PASS: three guarded native state/flag helpers, prior entries and inputs retained, twelve state checkpoints, screenshot comparison, copied-save load, four active palette cases, Ghidra names and unchanged original cards.')
