...
R=Path(__file__).resolve().parents[1];O=R/'coverage/frame-helpers-native';O.mkdir(parents=True,exist_ok=True)
runpy.run_path(str(R/'scripts/prepare_scene_callbacks_native.py'),run_name='__main__')
inputs=json.loads((R/'startup-scene-callbacks-inputs.json').read_text());manifest=[]
...
targets=[(0x80199440,0x80199558,'bof3_update_primary_scene_records'),(0x8019625c,0x8019651c,'bof3_apply_palette_adjustment_records'),(0x8019a0e4,0x8019a194,'bof3_dispatch_small_scene_record_updates')]
for lo,hi,name in targets:
 ...
 assert struct.unpack_from('<I',code)[0]>>16==0x27bd
 ...
 assert indirect==(['0x8019A15C'] if lo==0x8019a0e4 else [])
 live=O/'baseline'/f'live-code-{lo:08X}.bin';assert live.read_bytes()==code
 callers=[{'section':'GAME','pc':x['from']} for x in refs if x['to']==f'{lo:08x}' and 'CALL' in x['type']];assert callers
 ...
 manifest.append({'section':'GAME','input':'GAME-section-00-80195800.bin','image_base':f'0x{base:08X}','source_sha256':sha,'name':name,'entry':f'0x{lo:08X}','end_exclusive':f'0x{hi:08X}','observed_hotspots':[f'0x{lo:08X}'],'resume_dispatch_entries':[f'0x{pc:08X}' for pc in resumes],'case_dispatch_entries':[],'bytes':len(code),'sha256':hashlib.sha256(code).hexdigest(),'crc32':f'0x{zlib.crc32(code):08X}','direct_dependencies':sorted({x['target'] for x in direct}),'direct_calls':direct,'indirect_call_sites':indirect,'direct_call_sites':callers,'live_code_evidence':str(live.relative_to(R)),'boundary_evidence':'Complete Ghidra body and exact loaded RAM match; preceding return/delay slot; stack prologue; complete own return/delay slot; internal branch targets. Indirect callback table reads retained.'})
(R/'startup-frame-helpers-inputs.json').write_text(json.dumps(inputs,indent=2)+'\n');(O/'recipes.json').write_text(json.dumps({'routines':manifest,'total_new_instruction_bytes':sum(x['bytes'] for x in manifest)},indent=2)+'\n')
print('Prepared three frame helpers:',sum(x['bytes'] for x in manifest),'instruction bytes')
